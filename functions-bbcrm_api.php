...
function bbcrm_add_bbcrm_rewrite() {
	global $wp_rewrite,$wp_query;	
		//flush_rewrite_rules();		
		add_rewrite_rule('^bbcrm\/newpass/(.+)\/(.+)\/?','index.php?newpass={"useremail":"$matches[1]","newpass":"$matches[2]"}','top'); 
		...
		add_rewrite_endpoint( 'newpass', EP_ALL );
  
}
add_action( 'init', 'bbcrm_add_bbcrm_rewrite', 2,0 );

function bbcrm_newpass_api() {
	    global $wp_query;
	    
	    if ( !isset( $wp_query->query_vars['newpass'] ) )
		return;

		$newpass = json_decode(stripslashes($wp_query->query_vars['newpass']));
...
add_action( 'template_redirect', 'bbcrm_newpass_api' );
